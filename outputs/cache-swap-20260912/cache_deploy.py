...
f=Path('/userdata/rtctrl-face-video');d=f/'cache-swap'
assert len(json.loads((d/'matched-ab/results.json').read_text()))==2
protected=[f/'gallery.json',f/'models/detector.rknn',f/'models/recognizer.rknn']
hashes={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in protected}
shutil.copy2(d/'rtctrl_face_video_prefetch',f/'rtctrl_face_video.next');(f/'rtctrl_face_video.next').chmod(0o755)
...
with (d/'final-pmu.txt').open('w') as log:subprocess.run(['perf','stat','-e','task-clock,major-faults,minor-faults','-p',pid,'--','sleep','20'],stdout=log,stderr=log,check=True)
assert hashes=={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in protected}
(d/'manifest.json').write_text(json.dumps({'binary_sha256':hashlib.sha256((f/'rtctrl_face_video').read_bytes()).hexdigest(),'protected_hashes_unchanged':hashes,'pid':pid,'long_soak':False},indent=2));print('DONE',flush=True)
