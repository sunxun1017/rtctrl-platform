...
f=Path('/userdata/rtctrl-face-video');d=f/'fps30-test';out=d/'copy-ab';out.mkdir(exist_ok=True)
subprocess.run(['sh','stop-video.sh'],cwd=f,check=True)
p=None;results=[]
try:
    for i,name in enumerate(('final','copy','copy','final')):
        case=out/(str(i)+'-'+name);case.mkdir(exist_ok=True)
        exe=d/('rtctrl_face_video_'+name);exe.chmod(0o755)
        args=[str(exe),'--device','/dev/video31','--detector','models/detector.rknn','--recognizer','models/recognizer.rknn','--gallery','gallery.json','--threshold','0.5','--input-type','native-fp16','--jpeg-encoder','turbojpeg','--yuv-matrix','bt601','--yuv-range','full','--frame-converter','rga','--jpeg-mode','async']
