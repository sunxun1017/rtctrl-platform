...
int main(int argc, char** argv) {
    try {
        if (argc != 3) throw std::runtime_error("usage: npu_compare model.rknn repeats");
        ...
        std::cout << "input deterministic float32 0..255; no preprocessing/accuracy claim; timings_us\n";
        ...
        for (int pair=-3; pair<repeats; ++pair) {
            for (int slot=0; slot<2; ++slot) {
                bool prealloc=((pair+4)%2==0)?slot==1:slot==0;
                ...
                if (pair>=0) std::cout << "sample pair=" << pair << " prealloc=" << prealloc << " input_us=" << us(t0,t1) << " run_us=" << us(t1,t2) << " get_us=" << us(t2,t3) << " copy_us=" << us(t3,t4) << " release_us=" << us(t4,t5) << " total_us=" << us(t0,t5) << " max_abs_error=" << error << '\n';
            }
        }
