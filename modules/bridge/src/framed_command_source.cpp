...
transport::TransportStatus FramedCommandSource::open() noexcept {
    reset_link_session();
    const auto status = transport_.open();
    open_ = status == transport::TransportStatus::Ok;
    return status;
}
...
bool FramedCommandSource::poll(std::int64_t now_ns,
                               model::ControlTarget& target) noexcept {
    if (!open_ || now_ns < 0 || policy_.max_lease_us == 0 ||
        policy_.max_frames_per_poll == 0) {
        return false;
    }
    ...
    const auto io =
        transport_.try_receive(rx_.data() + rx_size_, kRxCapacity - rx_size_);
    if (io.status == transport::TransportStatus::Ok) {
    ...
    } else if (io.status != transport::TransportStatus::WouldBlock &&
               io.status != transport::TransportStatus::Timeout) {
        ++metrics_.transport_errors;
    }
    ...
    while (rx_size_ > 0 && frames_seen < policy_.max_frames_per_poll &&
           parse_steps < kRxCapacity) {
        ++parse_steps;
        ...
        if (last_sender_time_ns_ != 0 &&
            envelope.sender_time_ns < last_sender_time_ns_) {
        ...
        // Convert the relative wire lease into the receiver's monotonic clock
        // domain.
        latest.created_time_ns = now_ns;
        latest.valid_until_ns =
            now_ns + static_cast<std::int64_t>(lease_us) * 1000LL;
