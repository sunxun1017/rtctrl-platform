...
class DynamixelProtocol2 final : public IActuatorProtocol {
  public:
    ...
    ActuatorProtocolStatus encode_startup(std::int64_t now_ns,
                                          ActuatorPacketBatch& packets) noexcept override;
    ActuatorProtocolStatus decode_feedback(std::int64_t now_ns, const ActuatorPacketBatch& packets,
                                           model::SensorFrame& output) noexcept override;
    ActuatorProtocolStatus encode_arm(std::int64_t now_ns,
                                      ActuatorPacketBatch& packets) noexcept override;
    ActuatorProtocolStatus encode_command(std::int64_t now_ns, const model::CommandFrame& input,
                                          ActuatorPacketBatch& packets) noexcept override;
    ActuatorProtocolStatus encode_safe_stop(std::int64_t now_ns,
                                            ActuatorPacketBatch& packets) noexcept override;

    bool valid() const noexcept {
        return valid_;
    }
    static bool feedback_window(const DynamixelJointProfile& joint, std::uint16_t& start,
                                std::uint16_t& length) noexcept;

  private:
    bool append_packet(std::uint8_t id, std::uint8_t instruction, const std::byte* parameters,
    ...
    bool append_positions(const model::CommandFrame& input, ActuatorPacketBatch& packets) noexcept;
    bool append_individual_write(std::size_t joint_index, DynamixelRegister reg, std::int32_t value,
                                 ActuatorPacketBatch& packets) noexcept;
    bool append_homogeneous_sync_write(DynamixelRegister reg,
