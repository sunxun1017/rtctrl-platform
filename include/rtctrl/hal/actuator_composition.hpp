...
inline IActuatorLink* select_actuator_link(
    ActuatorLinkBackend backend,
    const ActuatorLinkProviders& providers) noexcept {
  switch (backend) {
    case ActuatorLinkBackend::Serial: return providers.serial;
    case ActuatorLinkBackend::CanFd: return providers.can_fd;
    case ActuatorLinkBackend::IghEthercat: return providers.igh_ethercat;
  }
  return nullptr;
}

inline ActuatorDependencies inject_actuator_dependencies(
    ActuatorLinkBackend backend, const ActuatorLinkProviders& providers,
