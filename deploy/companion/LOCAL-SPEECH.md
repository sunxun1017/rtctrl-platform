...
本板Main Mic接INPUT2/ADCR（右ADC）。录音用独立`rtctrl_es8389_capture`，
`ttable.0.1 1`取右声道；播放继续用原`rtctrl_es8389`，不要把录音的单声道映射套给扬声器。
固件PGA支持0–42dB，控制台按6dB步长暴露。现场暂用36dB、ADC数字音量0dB、ALC关闭。
增益已验证能放大输入，但近距离语音仍偏弱，不能声称硬件电平问题已完全解决。
