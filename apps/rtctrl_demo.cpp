...
extern "C" void handle_stop_signal(int) { stop_requested = 1; }
...
bool parse_options(int argc, char** argv, Options& options) {
  for (int i = 1; i < argc; ++i) {
    if (std::strcmp(argv[i], "--duration") == 0 && parse_int(argc, argv, i, options.duration_seconds)) {
    } else if (std::strcmp(argv[i], "--io-cpu") == 0 && parse_int(argc, argv, i, options.io_cpu)) {
    } else if (std::strcmp(argv[i], "--control-cpu") == 0 && parse_int(argc, argv, i, options.control_cpu)) {
    } else if (std::strcmp(argv[i], "--io-priority") == 0 && parse_int(argc, argv, i, options.io_priority)) {
    } else if (std::strcmp(argv[i], "--control-priority") == 0 && parse_int(argc, argv, i, options.control_priority)) {
    } else if (std::strcmp(argv[i], "--fault-after-ms") == 0 && parse_int(argc, argv, i, options.fault_after_ms)) {
