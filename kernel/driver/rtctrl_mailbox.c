...
static void rtctrl_free_dma(void *data)
{
	...
	if (mb->dma_cpu && !mb->preserve_dma)
		dma_free_coherent(mb->dev, mb->dma_bytes, mb->dma_cpu,
				  mb->dma_handle);
}
...
static int rtctrl_stop_locked(struct rtctrl_mailbox_dev *mb, int final_state)
{
	...
	if (atomic_read(&mb->state) == RTCTRL_MB_STATE_STOPPING)
		atomic_set(&mb->state, error ? RTCTRL_MB_STATE_FAULTED :
			   final_state);
	spin_unlock_irqrestore(&mb->state_lock, flags);
	return error;
}
...
static enum hrtimer_restart rtctrl_watchdog_fn(struct hrtimer *timer)
{
	...
	expires = min(mb->last_kick_ns + mb->watchdog_timeout_ns,
		      mb->command_deadline_ns);
	if (now >= expires) {
		rtctrl_hw_disable_locked(mb);
...
static irqreturn_t rtctrl_irq(int irq, void *data)
{
	...
	if (!(pending & RTCTRL_IRQ_FAULT) &&
	    !(status & RTCTRL_MB_STATUS_FAULT))
		return IRQ_HANDLED;
	spin_lock_irqsave(&mb->state_lock, flags);
	state = atomic_read(&mb->state);
	if (state != RTCTRL_MB_STATE_DEAD &&
	    state != RTCTRL_MB_STATE_STOPPING &&
	    state != RTCTRL_MB_STATE_SUSPENDED) {
		rtctrl_hw_disable_locked(mb);
		atomic_set(&mb->state, RTCTRL_MB_STATE_FAULTED);
		mb->staged_safe_command = false;
...
static void rtctrl_reset_session_locked(struct rtctrl_mailbox_dev *mb)
{
	...
	u32 feedback_producer =
		READ_ONCE(mb->layout->feedback.header.producer);
	...
	*safe_command = flags == RTCTRL_MB_COMMAND_FLAG_SAFE_STOP;
	...
	for (index = 0; index < joint_count; ++index) {
		...
		if (!rtctrl_float_finite(&joint->position) ||
		    ...
		    !rtctrl_float_finite(&joint->kp) ||
		    !rtctrl_float_finite(&joint->kd))
			return false;
		if (*safe_command &&
		    (!rtctrl_float_zero(&joint->velocity) ||
		     !rtctrl_float_zero(&joint->effort) ||
		     !rtctrl_float_zero(&joint->kp) ||
...
static long rtctrl_get_info(struct rtctrl_mailbox_dev *mb, void __user *user)
{
	...
	info = (struct rtctrl_mb_info) {
		...
		.watchdog_timeout_us =
			(u32)(mb->watchdog_timeout_ns / NSEC_PER_USEC),
...
static long rtctrl_arm(struct rtctrl_mailbox_dev *mb)
{
	...
	if ((status & (RTCTRL_MB_STATUS_READY | RTCTRL_MB_STATUS_FAULT)) !=
	    RTCTRL_MB_STATUS_READY || !mb->staged_safe_command ||
	    mb->command_deadline_ns <= now) {
		result = -EIO;
		goto unlock_state;
	}
	now = ktime_get_ns();
	if (mb->command_deadline_ns <= now ||
	    mb->command_deadline_ns - now <
		RTCTRL_MB_MIN_REMAINING_LEASE_NS) {
		result = -ETIME;
		goto unlock_state;
	}
	mb->last_kick_ns = now;
	expires = min(now + mb->watchdog_timeout_ns,
		      mb->command_deadline_ns);
	atomic_set(&mb->state, RTCTRL_MB_STATE_ARMED);
	hrtimer_start(&mb->watchdog, ns_to_ktime(expires),
...
static long rtctrl_disarm(struct rtctrl_mailbox_dev *mb)
{
	...
	spin_lock_irqsave(&mb->state_lock, flags);
	if (atomic_read(&mb->state) == RTCTRL_MB_STATE_STOPPING)
		atomic_set(&mb->state, state ? RTCTRL_MB_STATE_FAULTED :
			   RTCTRL_MB_STATE_DISARMED);
	spin_unlock_irqrestore(&mb->state_lock, flags);
	mutex_unlock(&mb->transition_lock);
	return state;
}
				  ...
				  const struct rtctrl_mb_command_frame *frame)
{
	...
	slot = &mb->layout->command.slot[(producer - 1U) &
						RTCTRL_MB_RING_MASK];
	*slot = *frame;
	dma_wmb();
				 ...
				 struct rtctrl_mb_feedback_frame *frame)
{
	...
	dma_rmb();
	*frame = mb->layout->feedback.slot[(producer - 1U) &
						  RTCTRL_MB_RING_MASK];
...
static long rtctrl_ioctl(struct file *file, unsigned int command,
			 unsigned long argument)
{
	...
	case RTCTRL_MB_IOC_SUBMIT_COMMAND:
		...
		if (copy_from_user(&ctx->command_staging, user,
				   sizeof(ctx->command_staging))) {
	...
	case RTCTRL_MB_IOC_READ_FEEDBACK:
		...
		result = copy_to_user(user, &ctx->feedback_staging,
				      sizeof(ctx->feedback_staging)) ? -EFAULT : 0;
		mutex_unlock(&ctx->feedback_lock);
		return result;
...
static int rtctrl_wait_dma_quiesced(struct rtctrl_mailbox_dev *mb)
{
	...
	return readl_poll_timeout(mb->regs + RTCTRL_REG_STATUS, status,
				  ...
				  1, RTCTRL_DMA_QUIESCE_TIMEOUT_US);
}
...
static int rtctrl_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(mb->reset))
		return dev_err_probe(dev, PTR_ERR(mb->reset),
				     "mailbox reset is required\n");
	error = rtctrl_assert_reset(mb);
	if (error)
		return dev_err_probe(dev, error,
				     "failed to assert mailbox reset\n");
	error = rtctrl_deassert_reset(mb);
	if (error)
		return dev_err_probe(dev, error,
				     "failed to deassert mailbox reset\n");
	...
	if (error)
		return dev_err_probe(dev, error,
				     "64-bit coherent DMA unavailable\n");
	mb->dma_bytes = PAGE_ALIGN(sizeof(struct rtctrl_mb_layout));
	mb->dma_cpu = dma_alloc_coherent(dev, mb->dma_bytes, &mb->dma_handle,
					 GFP_KERNEL);
	if (!mb->dma_cpu)
		return -ENOMEM;
	...
	if (device_property_read_u32(dev, "sx,joint-count",
				     &mb->joint_count) ||
	    mb->joint_count == 0 ||
	    mb->joint_count > RTCTRL_MB_MAX_JOINTS)
		return dev_err_probe(dev, -EINVAL,
				     "missing or invalid sx,joint-count\n");
	rtctrl_init_layout(mb);
	device_property_read_u32(dev, "sx,watchdog-timeout-us", &watchdog_us);
	if (watchdog_us < RTCTRL_MIN_WATCHDOG_US ||
	    watchdog_us > RTCTRL_MAX_WATCHDOG_US)
		return dev_err_probe(dev, -EINVAL,
	...
	if (error) {
		mutex_unlock(&mb->transition_lock);
		return dev_err_probe(dev, error,
				     "endpoint DMA did not quiesce\n");
	}
	atomic_set(&mb->state, RTCTRL_MB_STATE_DISARMED);
	mutex_unlock(&mb->transition_lock);
	mb->misc.minor = MISC_DYNAMIC_MINOR;
	mb->misc.name = devm_kasprintf(dev, GFP_KERNEL,
				      "rtctrl-mailbox-%s", dev_name(dev));
	if (!mb->misc.name)
		return -ENOMEM;
...
static void rtctrl_remove_common(struct platform_device *pdev)
{
	...
	mutex_lock(&mb->transition_lock);
	hrtimer_cancel(&mb->watchdog);
	if (!was_in_reset && rtctrl_wait_dma_quiesced(mb) &&
	    rtctrl_assert_reset(mb)) {
...
static void rtctrl_shutdown(struct platform_device *pdev)
{
	...
	if (!was_in_reset && rtctrl_wait_dma_quiesced(mb) &&
		...
		mb->preserve_dma = true;
		dev_crit(mb->dev,
			 "DMA active and reset failed during shutdown\n");
	} else if (rtctrl_assert_reset(mb)) {
