...
void test_official_sync_read_vector() {
  ...
  const std::array<std::byte, 6> parameters{
      std::byte{0x84}, std::byte{0x00}, std::byte{0x04},
  ...
  expect(encode_dynamixel_v2_packet(
             ...
             parameters.data(), parameters.size(), encoded.data(),
  ...
  expect(parser.push(encoded.data(), 5) &&
             !parser.pop(partial),
...
void test_stuffing_and_crc_recovery() {
  using namespace rtctrl::protocol;
  const std::array<std::byte, 5> parameters{
      std::byte{0x11}, std::byte{0xff}, std::byte{0xff},
      std::byte{0xfd}, std::byte{0x22}};
  std::array<std::byte, 64> encoded{};
  std::size_t encoded_size = 0;
  expect(encode_dynamixel_v2_packet(1, 0x03, parameters.data(),
                                    parameters.size(), encoded.data(),
                                    encoded.size(), encoded_size),
  ...
  expect(parser.pop(packet) && packet.parameter_size == parameters.size() &&
             std::memcmp(packet.parameters.data(), parameters.data(),
                         parameters.size()) == 0,
  ...
  std::array<std::byte, rtctrl::hal::kActuatorPacketPayloadCapacity>
      bounded_wire{};
  expect(encode_dynamixel_v2_packet(
             1, 0x03, worst_case.data(), worst_case.size(),
...
std::array<rtctrl::hal::DynamixelJointProfile,
           rtctrl::model::kJointCount>
make_profiles() {
  std::array<rtctrl::hal::DynamixelJointProfile,
             rtctrl::model::kJointCount> profiles{};
...
void test_motor_protocol_profile() {
  ...
  expect(protocol.encode_startup(0, outgoing) ==
  ...
  for (std::size_t index = 0; index < profiles.size(); ++index) {
    ...
    expect(rtctrl::protocol::encode_dynamixel_v2_packet(
               ...
               status_parameters.data(), status_parameters.size(),
               wire.payload.data(), wire.payload.size(), wire_size),
  ...
  expect(protocol.encode_safe_stop(0, outgoing) ==
             ActuatorProtocolStatus::Ok &&
             outgoing.size() == 1U,
         "safe stop broadcasts a batched Torque Disable");

  profiles[1].id = profiles[0].id;
  expect(!valid_dynamixel_protocol_config(
...
public:
  ...
  rtctrl::transport::IoResult try_receive(
      std::byte* destination, std::size_t capacity) noexcept override {
    if (!open_) {
  ...
  rtctrl::transport::IoResult try_send(
      const std::byte* source, std::size_t size) noexcept override {
    if (!open_) {
      return {rtctrl::transport::TransportStatus::Closed, 0, 0};
    }
