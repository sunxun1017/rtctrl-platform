...
HalStatus ProtocolActuatorHal::map_link_status(
    ActuatorLinkStatus status) noexcept {
  switch (status) {
    case ActuatorLinkStatus::Ok: return HalStatus::Ok;
    case ActuatorLinkStatus::WouldBlock: return HalStatus::NotReady;
    case ActuatorLinkStatus::Closed:
    case ActuatorLinkStatus::Error: return HalStatus::IoError;
  }
  return HalStatus::IoError;
}

HalStatus ProtocolActuatorHal::map_protocol_status(
    ActuatorProtocolStatus status) noexcept {
  switch (status) {
    case ActuatorProtocolStatus::Ok: return HalStatus::Ok;
    case ActuatorProtocolStatus::NotReady: return HalStatus::NotReady;
    case ActuatorProtocolStatus::InvalidData: return HalStatus::IoError;
  }
  return HalStatus::IoError;
}
...
HalStatus ProtocolActuatorHal::open_safe(std::int64_t now_ns) noexcept {
  ...
  armed_ = false;
  const auto startup_status = map_protocol_status(
      protocol_.encode_startup(now_ns, transmit_packets_));
...
HalStatus ProtocolActuatorHal::arm(std::int64_t now_ns) noexcept {
  ...
  transmit_packets_.clear();
  auto result = map_protocol_status(
      protocol_.encode_arm(now_ns, transmit_packets_));
  if (result != HalStatus::Ok) {
    return result;
  }
...
HalStatus ProtocolActuatorHal::write(
