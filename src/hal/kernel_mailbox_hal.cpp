...
KernelMailboxHal::~KernelMailboxHal() { close(); }
  ...
  if (info.abi_version != RTCTRL_MB_UAPI_ABI_VERSION ||
      (info.capabilities &
       (RTCTRL_MB_CAP_KERNEL_STAGED_IO | RTCTRL_MB_CAP_WATCHDOG)) !=
          (RTCTRL_MB_CAP_KERNEL_STAGED_IO | RTCTRL_MB_CAP_WATCHDOG) ||
      info.max_joints < model::kJointCount ||
      info.joint_count != model::kJointCount ||
      info.ring_depth != RTCTRL_MB_RING_DEPTH) {
    last_errno_ = EPROTO;
    close();
    return HalStatus::IoError;
  }
  watchdog_timeout_ns_ =
      static_cast<std::int64_t>(info.watchdog_timeout_us) * 1'000;
...
HalStatus KernelMailboxHal::arm(std::int64_t now_ns) noexcept {
  ...
  safe.valid_until_ns =
      now_ns + watchdog_timeout_ns_ +
      static_cast<std::int64_t>(RTCTRL_MB_MIN_REMAINING_LEASE_NS);
  ...
  if (ipc::encode_mailbox_command(safe, frame) !=
      ipc::MailboxFrameStatus::Ok) {
    last_errno_ = EPROTO;
    return HalStatus::IoError;
  }
...
HalStatus KernelMailboxHal::read(std::int64_t now_ns,
  ...
  if (::ioctl(fd_, RTCTRL_MB_IOC_READ_FEEDBACK, &frame) != 0) {
    ...
    if (error == EAGAIN && have_feedback_ &&
      last_feedback_.sample_time_ns <= now_ns &&
  ...
  const auto status = ipc::decode_mailbox_feedback(
      frame, now_ns, config_.max_feedback_age_ns, output);
...
HalStatus KernelMailboxHal::write(
    std::int64_t now_ns, const model::CommandFrame& input) noexcept {
  if (!opened_ || !armed_ || input.created_time_ns > now_ns ||
      input.valid_until_ns <= now_ns) {
    last_errno_ = EAGAIN;
    return HalStatus::NotReady;
  }
  rtctrl_mb_command_frame frame{};
  if (ipc::encode_mailbox_command(input, frame) !=
      ipc::MailboxFrameStatus::Ok) {
    last_errno_ = EPROTO;
    return HalStatus::IoError;
  }
