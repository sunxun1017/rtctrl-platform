...
SocketCanFdTransport::SocketCanFdTransport(
    const char* interface_name, SocketCanFdConfig config) noexcept
    : config_(config) {
...
TransportStatus SocketCanFdTransport::open() noexcept {
  ...
  if (::setsockopt(fd_, SOL_CAN_RAW, CAN_RAW_FD_FRAMES, &enabled,
                   sizeof(enabled)) != 0 ||
      ::setsockopt(fd_, SOL_CAN_RAW, CAN_RAW_LOOPBACK, &loopback,
                   sizeof(loopback)) != 0 ||
      ::setsockopt(fd_, SOL_CAN_RAW, CAN_RAW_RECV_OWN_MSGS, &receive_own,
  ...
  if (config_.receive_error_frames) {
    ...
    if (::setsockopt(fd_, SOL_CAN_RAW, CAN_RAW_ERR_FILTER, &error_mask,
                     sizeof(error_mask)) != 0) {
      last_error_ = errno;
      close();
      return TransportStatus::Error;
  ...
  if (filter_count_ != 0U) {
    ...
    if (::setsockopt(fd_, SOL_CAN_RAW, CAN_RAW_FILTER, native_filters.data(),
                     bytes) != 0) {
      last_error_ = errno;
      close();
      return TransportStatus::Error;
...
CanIoResult SocketCanFdTransport::try_receive(CanFrame& output) noexcept {
  ...
  for (cmsghdr* header = CMSG_FIRSTHDR(&message); header != nullptr;
    ...
    if (header->cmsg_level == SOL_SOCKET && header->cmsg_type == SCM_TIMESTAMPNS &&
      ...
      next.timestamp_ns = static_cast<std::int64_t>(timestamp.tv_sec) *
                              1'000'000'000LL +
                          timestamp.tv_nsec;
      break;
    }
