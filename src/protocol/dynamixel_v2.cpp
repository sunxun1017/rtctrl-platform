...
bool header_at(const std::byte* data) noexcept {
  return data[0] == kHeader[0] && data[1] == kHeader[1] &&
         data[2] == kHeader[2] && data[3] == kHeader[3];
}

}  // namespace

std::uint16_t dynamixel_v2_crc16(const std::byte* data,
...
bool encode_dynamixel_v2_packet(
    std::uint8_t id, std::uint8_t instruction, const std::byte* parameters,
    std::size_t parameter_size, std::byte* output, std::size_t capacity,
